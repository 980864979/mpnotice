...
public class TdConvHelper
{
    ...
    throws TdExprException
  {
    ...
    throw new TdExprException("错误提示信息: 待转换的bin-[" + bin + 
      "], 包含非法二进制形式.");
  }
  ...
  public static String hex2binary(char hex) throws TdExprException {
    switch (hex) {
    case '0':
      return "0000";
    case '1':
    ...
    case '^':
    case '_':
    case '`': } throw new TdExprException("十六进制转为二进制时, 有非法十六制字符:" + hex);
  }
  ...
  public static String binary2hex(String binary)
  {
    ...
    for (int i = 0; i < binLen; i += 4) {
      hexString = hexString + Integer.toHexString(Integer.valueOf(
        binary.substring(i, i + 4), 2).intValue());
    }
    return hexString;
  }

  public static String byte2String(byte val) throws TdExprException {
    byte[] arrVal = { val };
    return byte2String(arrVal, "ISO-8859-1");
  }

  public static String byte2String(byte[] val, String charset) throws TdExprException
  {
    try {
      return new String(val, charset); } catch (UnsupportedEncodingException e) {
    }
    throw new TdExprException("不支持指定的编码转换");
  }
    ...
    throws TdExprException
  {
    ...
    {
      throw new TdExprException("asc2bin执行出错, 一个ASCII值 + [" + strAsc + 
        "],转为对应的字符时失败."); } catch (UnsupportedEncodingException e) {
    }
  ...
  public static String convFlags(String equiv, byte flags) {
    ...
    int bit = 128; for (int i = 0; bit != 0; i++) {
      if ((flags & bit) != 0) {
        strb.setCharAt(0, '*');
        strb.append(chs[i]);
      }
